...
服务名和描述必须包含 `dev`；不开放 `0.0.0.0`，不引用生产目录。增加 `NoNewPrivileges=true`、`PrivateTmp=true`、`ProtectSystem=full` 与仅允许 `/opt/catmap-dev` 写入的 systemd 限制。
...
保留与生产脚本一致的本地依赖检查、打包、远程 `.venv` 管理、`alembic upgrade head`、systemd daemon-reload/restart、`nginx -t` 和 HTTPS health check 的行为，但所有临时归档和远程路径使用 `catmap-dev` 名称。上传文件必须进入 `/opt/catmap-dev` 下随机、root-only 的临时目录；远端脚本用 `trap` 无条件清理，不能把环境文件留在可预测的 `/tmp` 路径。
...
远程 shell 仅操作 `/opt/catmap-dev/backend`，安装 `catmap-backend-dev.service` 和 `catmap-dev.conf`。在删除旧代码前，检查计算后的目录和文件名完全等于开发固定值；保留开发目录中的 `.env`、`.venv`、`uploads`。首次部署时创建 `catmap-dev` 系统用户并在安装后将开发目录所有权交给该用户。Nginx 测试或 reload 失败时必须恢复开发 vhost 及其 `sites-enabled` 链接状态；不引用或覆盖生产 unit/Nginx 文件。
...
跟踪实际 `frontend/.env.development`，文件只含非敏感开发 API 值：

```dotenv
VITE_API_BASE_URL=https://dev-api.trmx.fun/api/v1
```

不改 `frontend/.env.example` 中的生产示例。`frontend/.env.development.example` 保留相同非敏感说明；不提交 `.env.development.local`。

在 `.gitignore` 的现有 `.env.*` 规则后增加 `!frontend/.env.development` 和 `!frontend/.env.development.example`，使非敏感开发配置/示例可被跟踪；运行 `git check-ignore -q -- frontend/.env.development`，预期返回未忽略。真实 `.env.development.local` 必须继续被忽略。

在 `frontend/package.json` 将通用 `build` 转发到 `build:mp-weixin:dev`，并将默认 `build:mp-weixin` 与别名 `build:mp-weixin:dev` 固定为 `uni build -p mp-weixin --mode development`；新增显式 `build:mp-weixin:prod`：`uni build -p mp-weixin --mode production`。开发小程序必须使用通用/默认命令或 `:dev` 别名；上线前生产验证只能使用 `:prod`。构建后扫描产物，开发包必须包含 `https://dev-api.trmx.fun/api/v1` 且不包含生产 API 地址，生产包则相反。
...
Run: `powershell -ExecutionPolicy Bypass -File scripts/deploy-backend-dev.ps1 -ServerHost <server-host> -EnvFile backend/.env`

Expected: 只创建/更新 `/opt/catmap-dev/backend`、`catmap-backend-dev`、开发 vhost；Alembic 对 `catmap_dev` 成功；开发 health 返回 200。

上线前的小程序生产验证使用：`npm run build:mp-weixin:prod`；不得用默认构建命令替代。
...
在 `docs/开发进度.md` 填入实际时间、部署提交、验证命令结果与未验证的小程序人工项。脚本不保留旧版本归档；如需回退，只允许停止开发 unit、恢复开发 vhost，或从已知上一个提交重新执行开发部署，不能回滚或重启生产服务。
