      ...
      <scroll-view class="point-scroll" scroll-y :show-scrollbar="false">
        <view class="point-list-body">
          ...
          <view v-else-if="loadState === 'error'" class="state-box">
            ...
            <button class="retry-button" hover-class="button-hover" @tap="loadLandmarkPoints">
              重新加载
            </button>
          </view>
...
async function loadLandmarkPoints() {
  ...
  try {
    const data = await getMapPoints(token, { point_types: "landmark" });
